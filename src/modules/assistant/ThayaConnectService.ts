...
import { ThayaConnectChat } from './schemas/ThayaConnectChatSchema';

@Injectable()
export default class ThayaConnectService {
    private readonly logger: Logger = new Logger('ThayaConnectService');
    ...
    constructor(
        ...
        @InjectModel(ThayaConnectChat.name)
        private readonly thayaConnectChatModel: Model<ThayaConnectChat>,
    ...
    async handleIncomingMessage(
        ...
        await this.thayaConnectChatModel.updateOne(
            { _id: chatId },
            { updatedAt: new Date() },
        );
...
`.trim();
    ...
    private async findUserChatAndCreateIfNotExists(
        userId: string,
    ): Promise<ThayaConnectChat> {
        let chat = (
            await this.thayaConnectChatModel.findOne({ userId }).exec()
        )?.toObject();
        ...
        if (!chat) {
            ...
            chat = await this.thayaConnectChatModel.create({
                _id: new mongoose.Types.ObjectId(),
                createdAt: new Date(),
                updatedAt: new Date(),
                userId,
